...
// NvidiaProvider wraps the OpenAI SDK configured for Nvidia NIM.
...
// NewNvidiaProvider initializes an Nvidia client round-robin cluster.
func NewNvidiaProvider(apiKeys []string, model string) (*NvidiaProvider, error) {
	if len(apiKeys) == 0 {
...
func (p *NvidiaProvider) SendMessage(ctx context.Context, messages []types.Message, toolDecls []*types.ToolDeclaration) (*types.LLMResponse, error) {
	...
	log.Printf("[LLM] Sending %d messages to Nvidia model %s with %d tools (using key idx %d)", len(oaiMessages), p.model, len(toolDecls), idx)

	resp, err := client.CreateChatCompletion(ctx, req)
	if err != nil {
		return nil, fmt.Errorf("nvidia API error (key %d): %w", idx, err)
	}

	return p.parseResponse(resp)
}

func (p *NvidiaProvider) parseResponse(resp openai.ChatCompletionResponse) (*types.LLMResponse, error) {
	result := &types.LLMResponse{Done: true}

	if len(resp.Choices) == 0 {
		return nil, fmt.Errorf("empty response from Nvidia")
	}
